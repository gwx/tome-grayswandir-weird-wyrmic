...
newTalent {
	...
	action = function(self, t)
		...
		if not actor.dead and actor.life / actor.max_life > 0.05 then
			if not rng.percent(util.getval(t.swallow_percent, self, t, actor)) then return true end
		...
		self:setEffect('EFF_WEIRD_SWALLOW', duration, {
										 ...
										 strength = util.getval(t.strength, self, t),
										 combat_physresist = util.getval(t.combat_physresist, self, t),})
		return true
	end,
...
This will also give you the a stack of the #UMBER#Swallow#LAST# buff for each rank of the enemy, up to your maximum of %d stacks. You will get %.1f #SLATE#[wil]#LAST# strength and %.1f #SLATE#[wil]#LAST# physical save per stack. #UMBER#Swallow#LAST# will last for %d turns, which will refresh every time it is applied.]])
			:format(util.getval(t.damage, self, t) * 100,
							util.getval(t.swallow_percent, self, t),
							util.getval(t.max_stacks, self, t),
							util.getval(t.strength, self, t),
							util.getval(t.combat_physresist, self, t),
...
While active, whenever your #UMBER#Swallow#LAST# runs out of time, your equilibrium will increase by %d #SLATE#[str]#LAST# and your #UMBER#Swallow#LAST# will lose 1 stack and have its duration reset, provided you pass an equilibrium check.]])
			:format(util.getval(t.duration, self, t),
							util.getval(t.max_stacks, self, t),
							util.getval(t.bite_chance, self, t),
							util.getval(t.bite_damage, self, t) * 100,
...
newTalent {
	...
	passives = function(self, t, p)
		self:talentTemporaryValue(p, 'combat_physcrit', util.getval(t.physcrit, self ,t))
	end,
	action = function(self, t)
		self:setEffect('EFF_BURROW', util.getval(t.duration, self, t), {})
		return true
	end,
	info = function(self, t)
...
The sharpest of claws are needed to burrow - this will passively increase your physical critical chance by %d%%.]])
			:format(util.getval(t.duration, self, t), util.getval(t.physcrit, self, t))
	end,}
...
newTalent {
	...
	passives = function(self, t, p)
		self:effectTemporaryValue(
			p, 'equilibrium_on_damage', {PHYSICAL = util.getval(t.equilibrium_gain, self, t),})
		self:effectTemporaryValue(
			p, 'inc_damage', {PHYSICAL = util.getval(t.inc_damage, self, t),})
		self:effectTemporaryValue(
			p, 'resists_pen', {PHYSICAL = util.getval(t.resists_pen, self, t),})
	end,
...
newTalent {
	...
	action = function(self, t)
		...
		local damage = util.getval(t.damage, self, t)
		local duration = self:mindCrit(util.getval(t.duration, self, t))
...
newTalent {
	...
	action = function(self, t)
		local tg = util.getval(t.target, self, t)
		local damage = self:mindCrit(util.getval(t.damage, self, t) * self:combatDamage())
	...
	info = function(self, t)
		return ([[Slam your foot onto the ground, shaking the area around you in radius %d. Those caught in the area will be hit with %d physical damage #SLATE#[phys, mind crit]#LAST# and #LIGHT_UMBER#knocked back#LAST# %d #SLATE#[str]#LAST# tiles. This will randomly rearrange the terrain within the quake's radius.]])
			:format(util.getval(t.radius, self, t),
							dd(self, 'PHYSICAL', util.getval(t.damage, self, t) * self:combatDamage()),
...
Every enemy killed in the initial blast will give you 1 stack of #UMBER#Swallow#LAST#, if you already have it.]])
			:format(dd(self, 'PHYSICAL', util.getval(t.damage, self, t)),
